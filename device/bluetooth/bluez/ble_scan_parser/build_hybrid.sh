...
show_help() {
    cat << EOF
BLE Scan Parser Build Script (Tier 2 Hybrid Build)
...
if $DO_CLEAN; then
    ...
    rm -rf out/Default/obj/device/bluetooth/bluez/ble_scan_parser
    cargo clean -p ble-scan-parser
    log_success "Clean complete"
    exit 0
fi

log_info "=== BLE Scan Parser Hybrid Build ==="
echo ""
...
# Step 2: Build C++ dependencies
if ! $DO_CHECK; then
    log_info "Step 2/3: Building C++ dependencies..."
...
echo ""
log_info "Component: device/bluetooth/bluez/ble_scan_parser"
log_info "Build type: Hybrid (GN + Cargo)"
log_info "Status: Tier 2 - Complex C++ FFI"
